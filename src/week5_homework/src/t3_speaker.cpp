...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    {
        char helpString[40] = "太阳当空照，花儿对我笑。";
        text = helpString;
        std::cout<<text<<std::endl;
    }
    ...
    else if(dataString.compare("向前。") == 0)
    {
        char helpString[40] = "太阳当空照，花儿对我笑。";
        text = helpString;
        std::cout<<text<<std::endl;
    }
    else if(dataString.compare("停车。") == 0)
    {
        char helpString[40] = "太阳当空照，花儿对我笑。";
        text = helpString;
        std::cout<<text<<std::endl;
...
{
	int         ret                  = MSP_SUCCESS;
        const char* login_params         = "appid = 5be831a9, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
	...
	printf("\n###########################################################################\n");
	printf("## 按任意键后按Enter来激活voice ##\n");
	printf("## 可用命令：向前。 向后。 向左。 向右。停车。  ##\n");
	printf("## 如果执行正确，会发出太阳当空照，花儿对我笑的音频  ##\n");
	printf("###########################################################################\n\n");
