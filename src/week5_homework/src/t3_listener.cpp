...
int main(int argc, char* argv[])
{
    ...
    while(ros::ok())
    {
        ...
        if (wakeupFlag){
            ROS_INFO("Wakeup...");
            int ret = MSP_SUCCESS;
            const char* login_params = "appid = 5be831a9, work_dir = .";
        ...
        if(resultFlag){
            ...
            std_msgs::String msg;
            msg.data = g_result;
            std::string gresult = g_result;
            geometry_msgs::Twist vel_msg;
            if(gresult.compare("向后。") == 0){
                vel_msg.linear.x = -0.2;
            }
            else if(gresult.compare("向左。") == 0)
            {
                vel_msg.linear.x = 0.2;
                vel_msg.angular.z = 0.2;
            }
            else if(gresult.compare("向右。") == 0)
            {
                vel_msg.linear.x = 0.2;
                vel_msg.angular.z = -0.2;
            }
            else if(gresult.compare("向前。") == 0)
            {
                vel_msg.linear.x = 0.2;
            }
            else if(gresult.compare("停车。") == 0)
            {
            }
